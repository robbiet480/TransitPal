...
extension Data {
    ...
    func getUnsignedInteger(at index: Int, length: Int = 4, bigEndian: Bool = true) -> UInt32 {
        let data: UInt32 = self.subdata(in: index ..< (index + length)).withUnsafeBytes { rawPointer in
            return rawPointer.bindMemory(to: UInt32.self).baseAddress!.pointee
        }
    ...
    func getUnsignedLong(at index: Int, length: Int = 8, bigEndian: Bool = true) -> UInt64 {
        let data: UInt64 = self.subdata(in: index ..< (index + length)).withUnsafeBytes { rawPointer in
